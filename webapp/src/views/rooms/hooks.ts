...
export const useRoomHooks = () => {
  ...
  const { state: roomsState, api: roomsApi } = useFetch<Room[]>(
    url,
    {
  ...
  useEffect(() => {
    ...
    if (miscRoomState.error) {
      logger.error("error fetching misc led strips", {
        error: miscRoomState.error,
      });
    }
    const interval = setInterval(() => {
      miscRoomApi.refetch();
...
export const useGetRoom = (roomId: number) => {
  const url =
    BACKEND_ROOT_URL + "rooms/" + roomId || 0 + "?include=led_strip_devices";
